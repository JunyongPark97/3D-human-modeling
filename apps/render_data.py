...
def render_prt_ortho(out_path, folder_name, subject_name, shs, rndr, rndr_uv, im_size, angl_step=4, n_light=1,
                     pitch=[0]):
    ...
    for p in pitch:
        for y in tqdm(range(0, 360, angl_step)):
            ...
            for j in range(n_light):
                ...
                cv2.imwrite(os.path.join(out_path, 'RENDER', subject_name, '%d_%d_%02d.jpg' % (y, p, j)),
                            255.0 * out_all_f)
                cv2.imwrite(os.path.join(out_path, 'MASK', subject_name, '%d_%d_%02d.png' % (y, p, j)),
                            255.0 * out_mask)
                ...
                cv2.imwrite(os.path.join(out_path, 'UV_RENDER', subject_name, '%d_%d_%02d.jpg' % (y, p, j)),
                            255.0 * uv_color)
...
if __name__ == '__main__':
    ...
    for i in range(len(folder_names)):
        ...
        render_prt_ortho(args.out_dir, args.input + '/' + folder_names[i], subject_name[i], shs, rndr, rndr_uv,
                         args.size, 1, 1, pitch=[0])
